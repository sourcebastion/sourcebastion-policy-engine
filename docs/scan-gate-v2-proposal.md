...
digest. A `project` bundle must be present: `bundles: []` and an
organization-only bundle set are errors. A compiled project bundle with no
policies is valid when every project row is disabled and no organization floor
applies. Unknown or missing fields are
